...
void loop()
{
  sensorValue = analogRead(0); // read analogue input pin 0 (to read the analogue input on Ao)
  digitalValue = digitalRead(11); // read and save the digital output on pin 11 of Arduino

  if (sensorValue > 400) {
    digitalWrite(13, HIGH); // if the analogue reading is greater than 400, then the LED turns ON
    tone(buzzer, 800, 1000); //play noise
    digitalWrite(12, HIGH);
    //song();
  } else {
    noTone(buzzer);
    digitalWrite(12, LOW); // if the analogue reading is less than 400, the LED turns OFF
  }

  XBeeSerial.print(sensorValue); // send the analogue reading over XBee
  Serial.print("CO Data Sent: ");
  Serial.print(sensorValue, DEC); // print the analogue reading to the serial monitor for debugging
  Serial.print("\n");
